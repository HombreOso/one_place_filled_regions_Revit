...
def is_polygon_ccw_then_revert(polygon, polygon_type):
    if len(polygon) >0:
        ...
        if ccw(polygon_x0, polygon_y0, polygon_x1, polygon_y1, polygon_x2, polygon_y2):
...
for filled_region in filled_regions:
    ...
    vertices = is_polygon_ccw_then_revert(vertices, 'filled region')


    for space_room in all_spaces_and_rooms:
        parameters_map = space_room.GetParameters('Number')[0].AsString()
        fr_param_map = filled_region.GetParameters('Area')[0].AsDouble()
        print('room number', parameters_map)
        print('filled region', fr_param_map)
        ...
        vertices_coordinates = is_polygon_ccw_then_revert(vertices_coordinates, 'room')
        try:

            print('do polygons overlap?: ', do_polygons_overlap(vertices, vertices_coordinates))
            if vertices_coordinates and do_polygons_overlap(vertices, vertices_coordinates):
